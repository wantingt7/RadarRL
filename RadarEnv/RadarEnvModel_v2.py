...
class RadarEnvModel(Env):
    ...
    def step(self, action):
        ...
        nid = np.random.choice(self.net_num, p = self.prob)
        temp = self.net_ensemble[nid](torch.FloatTensor(self.state.flatten()).float())
        if self.net_type == 1:
