...
# ---------------------------------- Utils -----------------------------------------
    ...
    """
    only for pre-trained fictitious models' testing & model-ensemble algorithms
    ...
    def get_jammer_act_distr(self, obs_next, pulse):
        ...
        for obs in obs_next:
            ...
            if self.jammer_type == 1:
                if round % 4 == 0:
                    action[4] = 1
                last_pulse = round // 4
                length = 4 * last_pulse
                check_state = layer1[:, 3*length+1:3*length+3]
                radar_action = np.argwhere(check_state == 1)
                if radar_action[0, 0] != radar_action[1, 0]:
                    action[3] = 1
                else:
                    action[radar_action[0, 0]] = 1
            elif self.jammer_type == 2:
                if round % 4 == 0:
                    action[4] = 1
                else:
                    last_pulse = round // 4
                    length = 4 * last_pulse
                    check_state = layer1[:, 3*length]
                    radar_action = np.argwhere(check_state == 1)
                    action[radar_action[0, 0]] = 1
            elif self.jammer_type == 3:
                if 0 <= round % 8 <= 2: 
                    action[4] = 1
                elif round % 8 == 3:
                    action[3] = 1
                    ...
                    last_pulse = round // 8
                    temp = list()
                    # only collect the latest first 6 radar's action
                    for i in range(3):
                        length = 8 * last_pulse + i
                        check_state = layer1[:, 3*length+1:3*length+3]
                        radar_action = np.argwhere(check_state == 1)
                        temp.append(radar_action[:, 0])
                    temp_ = np.array(temp).reshape(-1, 6)[0]
    ...
    def Env_choose(self, a, cpi):
        if self.jammer_type == 1:
            ...
            if cpi % 4 == 0:
                ...
                aj = np.array([0, self.last_pulse[0], 3])
        else:
            if 0 <= cpi % 8 <= 2:
                aj = np.array([1, 3, 3])
...
# ---------------------------------- Utils -----------------------------------------
    ...
    def test_accuracy(self, nid, test_collector, test_size, loss_fn_name):
    ...
    def net_selection(self, collector, test_size, is_select_max=False):
        collector.reset_env()
